    ...
    for ea in ['EA1']:
        ...
        for enemy in enemy_groups:
            ...
            for enemy in all_enemies:
                ...
                # path = f'best_solutions/{ea}/{enemies_folder}'
                # # List all files in the folder
                # files = os.listdir(path)
                # file_name = files[9]  # Only one file in the folder
                # weights = np.loadtxt(f'{path}/{file_name}')

                enemies = '367'
                ea = 'EA1'
                name = 'best_defeated.txt'
                _, _, _, weights = load_final_best_solution(enemies, ea, name)
