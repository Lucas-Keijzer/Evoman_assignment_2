...
class environm(Environment):
    ...
    #fitness aangepast!!
    def fitness_single(self, y=0.9, a=0.1):
        enemy_energy = self.get_enemylife()
        player_energy = self.get_playerlife()
        time = self.get_time()

        # Fitness calculation based on the new formula
        energy_term = y * (100 - enemy_energy) + a * player_energy
        if player_energy > 0:
            time_term = -np.log(time)
        else:
            time_term = np.log(time)

        fitness = energy_term + time_term
        return fitness
...
# crossover aangepast!!
def crossover(pop, fit_pop, alpha=0.5):
    ...
    for p in range(0, pop.shape[0], 2):
        ...
        # Randomly decide the number of offspring to generate (1-3)
        ...
        for f in range(0, n_offspring):
            ...
            for i in range(n_vars):
                d = abs(p1[i] - p2[i])
                lower_bound = min(p1[i], p2[i]) - alpha * d
                upper_bound = max(p1[i], p2[i]) + alpha * d
                offspring[f][i] = np.random.uniform(lower_bound, upper_bound)
            ...
            # Apply limits to offspring values (assuming 'limits' function handles boundary constraints)
            offspring[f] = np.array(list(map(lambda y: limits(y), offspring[f])))

            # Append the new offspring to the total_offspring array
